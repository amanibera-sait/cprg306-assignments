...
export default function NewItem({onAddItem}){
    ...
    return (
        <div className= "flex m-5">
            <div className="bg-gray-400 p-9 rounded-lg">
                <form className="flex flex-col items-center" onSubmit={handleSubmit}>
                    ...
                    <div className="flex flex-row items-center">
                        <label className="mt-4 pr-10">
                            <input className="border border-gray-300 rounded p-2 w-15" type="number" min={1} max={99} value={quantity} onChange={(event) => setQuantity(event.target.value)} />
                        </label>
                        <label className="mt-4">
